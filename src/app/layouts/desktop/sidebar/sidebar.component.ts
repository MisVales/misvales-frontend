...
import {
  ...
  NavItemData,
  NavGroupData,
} from '@shared/utils/navigation/navigation.config';
import { LucideAngularModule } from 'lucide-angular';
import { Subscription, filter } from 'rxjs';
import { filterNavigationItems } from '@core/authorization/navigation.permissions';
import { ShellNavigationService } from '../../../layouts/desktop/navigation/shell-navigation.service';
import { BrandLockupComponent } from '@shared/components/brand/brand-lockup/brand-lockup.component';

@Component({
  selector: 'app-sidebar',
  standalone: true,
  imports: [RouterLink, NgTemplateOutlet, LucideAngularModule, BrandLockupComponent],
...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  filteredNavGroups = computed<NavGroupData[]>(() => {
    ...
    return navigationGroupsForRoles(roles)
      .map((group) => ({
        ...group,
  ...
  filteredBottomItems = computed<NavItemData[]>(() => {
    ...
    ).filter((item) => item.id !== 'my-account' && item.id !== 'logout');
  });
  ...
  toggleGroup(item: NavItemData, parentId?: string): void {
    if (!parentId) {
      ...
      this.openGroupId.update((current) => (current === item.id ? null : item.id));
    } else {
  ...
  isActive(item: NavItemData): boolean {
    ...
    return this.matchesItemRoute(item, this.activeRoute());
  }
  ...
  private openAncestorsForRoute(url: string): void {
    // Buscar en navGroups
    for (const group of navigationGroupsForRoles(this.sessionStore.roles())) {
      for (const item of group.items) {
        if (this.itemContainsRoute(item, url)) {
          // Una página directa (por ejemplo, Auditoría general) no es un acordeón.
          // No debe reemplazar el grupo que el usuario abrió manualmente.
  ...
  private itemContainsRoute(item: NavItemData, url: string): boolean {
    if (item.route && this.matchesItemRoute(item, url)) {
      return true;
    }
    if (item.children) {
  ...
  private matchesItemRoute(item: NavItemData, url: string): boolean {
    if (!item.route) return false;

    const [urlWithoutFragment, currentFragment] = url.split('#', 2);
    const currentPath = urlWithoutFragment.split('?', 1)[0];
    const routeMatches = currentPath === item.route || currentPath.startsWith(item.route + '/');

    if (!routeMatches) return false;
    return item.fragment ? currentFragment === item.fragment : !currentFragment;
  }
}
