...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, HistoryPageHeaderComponent, HistoryFilterBarComponent],
  template: ` <section class="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
    <app-history-page-header
      [eyebrow]="isDistributor() ? 'Mi cuenta · crédito' : 'Distribuidoras · crédito'"
      [title]="isDistributor() ? 'Mi línea de crédito' : 'Líneas de crédito'"
      [description]="isDistributor() ? 'Consulta tu saldo, estado y solicitudes de incremento en un solo lugar.' : 'Historial de saldos, solicitudes y movimientos. La actividad más reciente aparece primero.'"
    />
    @if (!isDistributor()) {
      <app-history-filter-bar label="Filtros del historial de líneas de crédito">
      <input
        type="search"
        class="min-h-11 rounded-lg border border-slate-300 bg-white px-3"
        placeholder="Distribuidora o número"
        [ngModel]="lineSearch()"
    ...
    @if (loading()) {
      ...
      @if (selectedLine(); as line) {
        <section class="border-y border-slate-200 bg-white">
          @if (isDistributor()) {
            <div class="rounded-2xl bg-gradient-to-br from-emerald-900 to-emerald-700 p-5 text-white shadow-lg sm:p-6">
              <div class="flex flex-wrap items-start justify-between gap-3">
                <div><p class="text-xs font-bold uppercase tracking-[.14em] text-emerald-100">Estado de la línea</p><h2 class="mt-1 text-xl font-bold">{{ restrictionLabel(line) }}</h2></div>
                <span class="rounded-full bg-white/15 px-3 py-1 text-xs font-bold">{{ usedPercent(line) }}% en uso</span>
              </div>
          ...
          @if (line.capabilities?.can_request_increase && !isGeneralManager()) {
            <form
              ...
              @if (lineRequests().length) {
                <div class="mt-5 overflow-x-auto rounded-xl border border-slate-200">
                  <table class="min-w-full text-left text-sm">
                    <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th class="p-3">Solicitud</th><th class="p-3">Importe</th><th class="p-3">Estado</th><th class="p-3">Resultado</th></tr></thead>
                    <tbody>
                      @for (request of lineRequests(); track request.id) {
                        <tr class="border-t border-slate-100 align-top">
                          <td class="whitespace-nowrap p-3"><strong class="block text-slate-900">{{ request.request_number }}</strong><span class="text-xs text-slate-500">{{ request.requested_at | date: 'shortDate' }}</span></td>
                          <td class="whitespace-nowrap p-3 font-semibold tabular-nums">{{ request.requested_amount | currency: 'MXN' }}</td>
                          <td class="p-3"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-bold" [class]="statusClass(request.status)">{{ statusLabel(request.status) }}</span></td>
                          <td class="min-w-52 p-3 text-xs leading-5 text-slate-600">{{ requestNarrative(request) }}</td>
                        </tr>
                      }
                    </tbody>
                  </table>
...
export class LineasCreditoPageComponent implements OnInit {
  ...
  readonly filteredLines = computed(() => {
    const search = this.lineSearch().trim().toLocaleLowerCase('es-MX');
    if (!search) return this.lines();
    return this.lines().filter(
      (line) =>
        line.distributor.full_name.toLocaleLowerCase('es-MX').includes(search) ||
        line.distributor.distributor_number.toLocaleLowerCase('es-MX').includes(search),
    );
  });
