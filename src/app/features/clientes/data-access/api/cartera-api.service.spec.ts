...
describe('CarteraApiService', () => {
  ...
  it('envía el movimiento informativo canónico sin convertirlo en operación financiera',()=>{
    ...
    request.flush({data:{id:'m1',entry_type:'DEBT',amount:'250.0000',occurred_at:payload.occurred_at,note:'Seguimiento',recorded_by:'u1'}});
    expect(result.tipo).toBe('CHARGE'); expect(result.importe).toBe('250.0000');
  });
});
