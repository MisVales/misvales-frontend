...
@Component({
  selector: 'app-category-versions-page',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, UiStateComponent, CategoryVersionFormComponent],
  template: `
    <div class="p-6">
      <div class="flex justify-between items-center mb-6">
        <div>
          <button (click)="goBack()" class="text-blue-600 hover:underline text-sm mb-2">&larr; Volver a Categorías</button>
          <h1 class="text-2xl font-bold">Historial de Versiones</h1>
          <p class="text-gray-500">Categoría: {{ publicId }}</p>
        </div>
        <button
          *ngIf="canEdit()"
      ...
      <ng-container *ngIf="loadState() === 'content' || loadState() === 'empty'; else stateTemplate">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tasa Ganancia</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr *ngFor="let version of versions()">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ version.name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ version.distributorProfitRate }}</td>
        ...
        <div *ngIf="loadState() === 'empty'" class="text-center py-10 bg-white rounded-lg shadow">
          <p class="text-gray-500">No hay versiones para esta categoría.</p>
        </div>
      </ng-container>
      ...
      <!-- Modal de Creación de Nuevo Borrador (FE08.02) -->
      ...
      <div *ngIf="publishModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden">
          <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">Publicar Categoría</h3>
          </div>
...
export class CategoryVersionsPageComponent implements OnInit {
  ...
  readonly versions = signal<CategoryVersion[]>([]);
  ...
  selectedVersion: CategoryVersion | null = null;
  ...
  loadVersions() {
    ...
    this.categoriesService.getVersions(this.publicId, this.page(), this.perPage()).pipe(
      catchError(() => {
        this.loadState.set('error');
  ...
  goBack() {
    this.router.navigate(['/administrativa/categorias']);
  }
  ...
  onCreateDraft(data: CategoryFormData) {
    this.errorMessage.set(null);
    this.isSubmitting.set(true);
    this.categoriesService.createDraft(this.publicId, data.name, data.description, data.distributorProfitRate).pipe(
  ...
  // --- Publish Actions ---
  openPublishModal(version: CategoryVersion) {
  ...
  onPublish() {
    ...
    this.isSubmitting.set(true);
      ...
      this.categoriesService.publish(this.publicId, this.selectedVersion.versionPublicId, effectiveFromWithZone, reason).pipe(
        finalize(() => this.isSubmitting.set(false)),
        catchError(err => {
          console.error(err);
          const msg = err.error?.error?.message || 'Error al procesar la solicitud.';
  ...
  // --- Deactivate Actions ---
  openDeactivateModal(version: CategoryVersion) {
    this.selectedVersion = version;
  ...
  onDeactivate() {
    ...
    this.isSubmitting.set(true);
      ...
      this.categoriesService.deactivate(this.publicId, reason).pipe(
        finalize(() => this.isSubmitting.set(false)),
        catchError(err => {
          console.error(err);
          const msg = err.error?.error?.message || 'Error al procesar la solicitud.';
