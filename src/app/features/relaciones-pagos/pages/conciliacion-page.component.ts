...
import { RelationView, RelacionesApiService } from '../data-access/relaciones-api.service';

@Component({
  selector: 'app-conciliacion-page',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `<section class="space-y-8 p-6">
    <header>
      ...
      <div class="flex flex-wrap items-end justify-between gap-4">
        ...
        <div class="grid w-full gap-3 sm:w-auto sm:grid-cols-3">
          <label class="text-xs font-semibold text-gray-600"
            >Buscar<input
              class="mt-1 min-h-11 w-full rounded-lg border px-3 text-sm font-normal"
              [ngModel]="search()"
          ...
          <tbody class="divide-y">
            @for (item of movements(); track item.id) {
              <tr class="align-top hover:bg-gray-50">
                ...
                <td class="p-3">
                  {{ item.distributor_name || item.distributor_number || 'Sin asignar' }}
                </td>
      ...
      >
        ...
        <div class="mt-4 grid gap-4 lg:grid-cols-3">
          <label class="text-sm font-semibold text-blue-950"
            >Relación<select
              class="mt-1 min-h-11 w-full rounded-lg border bg-white px-3 font-normal text-gray-900"
        ...
        @for (item of clarifications(); track item.id) {
          <article class="rounded-xl border bg-white p-4">
            ...
            <dl class="mt-3 text-xs text-gray-500">
              ...
              <dd class="text-gray-800">{{ item.distributor_number || item.distributor_id }}</dd>
            </dl>
            <button
              class="mt-3 min-h-11 rounded-lg border px-3 text-xs font-semibold text-gray-800 hover:bg-gray-50"
        ...
        @if (canAuthorize()) {
          <label class="text-xs font-semibold text-gray-600"
            >Motivo de rechazo<input
              class="mt-1 min-h-11 rounded-lg border px-3 text-sm font-normal"
