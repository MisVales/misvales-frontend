...
describe('DistribuidorasApiService', () => {
  ...
  beforeEach(()=>{TestBed.configureTestingModule({imports:[HttpClientTestingModule],providers:[DistribuidorasApiService,{provide:API_CONFIG,useValue:{baseUrl:'https://api.example.test/api/v1'}}]});service=TestBed.inject(DistribuidorasApiService);http=TestBed.inject(HttpTestingController);});
  ...
  it('activa desde la solicitud autorizada con una versión publicada de categoría',()=>{
    ...
    const request=http.expectOne('https://api.example.test/api/v1/distributor-applications/app-1/activation');
  ...
  it('incluye lock_version en el cuerpo al cambiar categoría',()=>{
    ...
    const request=http.expectOne('https://api.example.test/api/v1/distributors/d1/category-assignments');
  ...
  it('desenvuelve la respuesta del detalle antes de mapearla',()=>{
    ...
    const request=http.expectOne('https://api.example.test/api/v1/distributors/d1');
  ...
  it('lista distribuidoras usando el origen configurado del API',()=>{
    service.listar(1,10).subscribe();
    const request=http.expectOne('https://api.example.test/api/v1/distributors?page=1&per_page=10');
    expect(request.request.method).toBe('GET');
    request.flush({data:[],meta:{current_page:1,last_page:1,per_page:10,total:0}});
  });
});
