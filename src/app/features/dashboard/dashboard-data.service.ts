...
export class DashboardDataService {
  ...
  private loadDistributor(): Observable<DashboardData> {
    return forkJoin({
      ...
      summary: this.operationDashboard.distributorSummary(),
    }).pipe(
      map(({ credit, vouchers, relations, points, summary }) => {
        ...
        return {
          kpis: [
            ...
            kpi('used', 'Capital utilizado', money(credit.used_balance), 'banknote', 'orange'),
            kpi(
              'available',
              'Línea disponible',
          ...
          sections: [
            ...
            },
            {
              ...
              summary: priorityRelation
                ? [
                    ...
                    {
                      id: 'balance',
                      label: 'Importe pendiente de la relación',
            ...
            },
            {
              ...
              icon: 'users-round',
              route: '/vales',
              routeLabel: 'Ver clientes en vales',
              emptyMessage: 'No hay saldos informativos por cobrar.',
              summary: [
                { id: 'collect', label: 'Total por cobrar a clientes', value: money(summary.portfolio.total_to_collect), tone: Number(summary.portfolio.total_to_collect) > 0 ? 'orange' : 'green' },
                { id: 'clients', label: 'Clientes con saldo', value: number(summary.portfolio.clients_with_balance) },
                { id: 'overdue', label: 'Registros informativamente vencidos', value: number(summary.portfolio.overdue_entries), tone: summary.portfolio.overdue_entries ? 'red' : 'green' },
              ],
            },
            {
              id: 'period-summary',
              ...
              description: `${date(summary.period_start)} – ${date(summary.period_end)}`,
              icon: 'calendar-range',
              emptyMessage: 'No hay movimientos en el periodo.',
              summary: [
                { id: 'profit', label: 'Ganancia generada de la distribuidora', value: money(summary.period.distributor_profit), tone: 'green' },
                { id: 'paid', label: 'Pagado a MisVales', value: money(summary.period.paid_to_misvales), tone: 'blue' },
                { id: 'recovered', label: 'Capital recuperado', value: money(summary.period.capital_recovered) },
                { id: 'points', label: 'Puntos disponibles', value: `${number(points.available_points)} pts`, tone: 'green' },
              ],
            },
            {
              id: 'recent-vouchers',
  ...
  return {
    ...
    kpis: [
      kpi(
        'period-portfolio',
        'Cartera del periodo',
        money(report.financial.portfolio_total),
        'briefcase-business',
        'blue',
        `${date(report.financial.period_start)} – ${date(report.financial.period_end)}`,
        '/relaciones-pagos/relaciones',
      ),
      kpi(
        'period-misvales',
        'Importe correspondiente a MisVales',
        money(report.financial.misvales_total),
        'receipt-text',
        'purple',
        `${number(report.financial.relations)} relaciones del periodo`,
        '/relaciones-pagos/relaciones',
      ),
      kpi(
        'period-received',
        'Total recibido por MisVales',
        money(report.financial.received_total),
        'circle-dollar-sign',
        'green',
        'Pagos aplicados durante el periodo',
        '/relaciones-pagos/pagos',
      ),
      kpi(
        'period-pending',
        'Total pendiente con MisVales',
        money(report.financial.pending_total),
        'clock-3',
        Number(report.financial.pending_total) > 0 ? 'orange' : 'green',
        'Saldo pendiente de relaciones del periodo',
        '/relaciones-pagos/relaciones',
      ),
      kpi(
        'period-overdue',
        'Total vencido con MisVales',
        money(report.financial.overdue_total),
        'triangle-alert',
        Number(report.financial.overdue_total) > 0 ? 'red' : 'green',
        `${number(report.delinquency.total)} distribuidoras con morosidad`,
        '/riesgo',
      ),
      kpi(
        'attention',
        'Operación que requiere atención',
        number(report.applications.pending + operation.reconciliation.pending + operation.reconciliation.manual_pending),
        'inbox',
        report.applications.pending + operation.reconciliation.pending + operation.reconciliation.manual_pending ? 'orange' : 'green',
        `${number(report.applications.pending)} solicitudes · ${number(operation.reconciliation.pending + operation.reconciliation.manual_pending)} conciliaciones`,
