...
export class ActivacionDistribuidoraPageComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try {
      ...
      this.error.set(e?.error?.error?.message ?? e?.error?.message ?? 'No fue posible preparar la activación.');
    } finally {
  ...
  async confirmarActivacion() {
    ...
    if (!application || this.form.invalid || this.activando() || application.status !== 'AUTHORIZED_PENDING_ACTIVATION') {
    ...
    this.error.set(null);
    try {
      const distributor = await firstValueFrom(this.api.activarSolicitud(application.id, this.form.getRawValue().category_version_id!));
      await this.router.navigate(['/distribuidoras', distributor.id]);
    } catch (e: any) {
      this.error.set(e?.error?.error?.message ?? e?.error?.message ?? 'No fue posible activar la distribuidora.');
    } finally {
  ...
  volver() {
    this.router.navigate(['/verificacion-distribuidoras/solicitudes-distribuidora', this.solicitud()?.id]);
  }
}
