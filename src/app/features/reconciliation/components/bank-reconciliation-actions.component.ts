...
@Component({
  ...
  template: `
    <section
      ...
      aria-labelledby="bank-actions-title"
    >
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 id="bank-actions-title" class="mt-1 text-xl font-bold text-slate-950">
            Conciliaciones pendientes
          </h2>
          <p class="mt-1 max-w-3xl text-sm text-slate-600">
            Procesa cada corte por separado. Se muestran del más nuevo al más antiguo.
          </p>
        </div>
        @if (loading()) {
          <span class="text-sm text-slate-500">Consultando cortes…</span>
        }
      </div>
      ...
      @if (!loading() && periods().length === 0) {
        <div
          class="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"
          role="status"
        >
          ...
          <article class="rounded-xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
            <div class="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h3 class="font-bold text-slate-950">
                  Conciliación #{{ period.reconciliation_number }}
                </h3>
                <p class="mt-1 text-sm text-slate-600">
                  Corte: {{ period.cutoff_at | date: 'dd/MM/yyyy HH:mm' }} ·
                  {{ period.relations }} relaciones · {{ period.distributors }} distribuidoras
                </p>
              </div>
              <div class="sm:text-right">
                ...
                ><strong class="text-lg text-slate-950">{{
                  period.pending_total | currency: 'MXN'
                }}</strong>
              </div>
            </div>
            @if (periodError(period.process_run_id)) {
              <div
                role="alert"
                class="mt-3 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800"
              >
                {{ periodError(period.process_run_id) }}
              </div>
            }
            @if (periodSuccess(period.process_run_id)) {
              <div
                role="status"
                class="mt-3 rounded-lg border border-green-200 bg-green-50 p-3 text-sm text-green-800"
              >
                {{ periodSuccess(period.process_run_id) }}
              </div>
            }
            ...
            <div class="mt-4 grid gap-4 md:grid-cols-2">
              <div class="rounded-lg border border-slate-200 bg-white p-4">
                ...
                <p class="mt-1 text-sm text-slate-600">
                  {{
                    movementCount(period.process_run_id) === null
                      ? 'Consultando movimientos…'
                      : 'Incluye ' +
                        movementCount(period.process_run_id) +
                        ' movimientos simulados de este corte.'
                  }}
                </p>
                <button
                  type="button"
                  class="mt-4 min-h-11 rounded-lg bg-slate-950 px-4 font-semibold text-white disabled:opacity-50"
                  [disabled]="isDownloading(period.process_run_id)"
                  (click)="download(period)"
                >
                  {{ isDownloading(period.process_run_id) ? 'Preparando…' : 'Descargar Excel' }}
                </button>
              </div>
              @if (canUpload) {
                <div class="rounded-lg border border-slate-200 bg-white p-4">
                  ...
                  <p class="mt-1 text-sm text-slate-600">
                    Selecciona el XLSX correspondiente únicamente a esta conciliación.
                  </p>
                  <label
                    class="mt-4 inline-flex min-h-11 cursor-pointer items-center rounded-lg border border-emerald-700 bg-white px-4 font-semibold text-emerald-800"
                    >Seleccionar Excel<input
                      type="file"
                      class="sr-only"
                      accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                      (change)="select(period.process_run_id, $event)"
                  /></label>
                  @if (selectedFileName(period.process_run_id); as selectedName) {
                    <div
                      class="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg bg-slate-50 p-3"
                    >
                      ...
                      <button
                        ...
                        [disabled]="isUploading(period.process_run_id)"
                        (click)="upload(period.process_run_id)"
                      >
                        {{
                          isUploading(period.process_run_id) ? 'Procesando…' : 'Conciliar archivo'
                        }}
                      </button>
                    </div>
                  }
                ...
                <div class="rounded-lg border border-slate-200 bg-white p-4">
                  <h4 class="font-bold text-slate-950">2. Carga operativa</h4>
                  <p class="mt-1 text-sm text-slate-600">
                    La carga de archivos está reservada a cajeras con una sucursal operativa
                    autorizada.
                  </p>
                </div>
              }
...
export class BankReconciliationActionsComponent {
  ...
  movementCount(id: string): number | null {
    return this.counts()[id] ?? null;
  }
  selectedFileName(id: string): string {
  ...
  isUploading(id: string): boolean {
    return this.uploading()[id] ?? false;
  }
  periodError(id: string): string {
    return this.errors()[id] ?? '';
  }
  periodSuccess(id: string): string {
    return this.successes()[id] ?? '';
  }
  ...
  select(id: string, event: Event): void {
    try {
      ...
      this.set(this.errors, id, 'No fue posible leer el archivo. Selecciona un XLSX válido.');
    }
  }
  ...
  private readSelectedFile(id: string, event: Event): void {
    ...
    this.set(this.errors, id, '');
    this.set(this.successes, id, '');
    if (!file) {
      this.clearSelectedFile(id);
      return;
    }
  ...
  download(period: PendingReconciliationPeriod): void {
    ...
    this.set(this.downloading, id, true);
    this.set(this.errors, id, '');
    this.api.exportSimulatedTransfers(id).subscribe({
      next: (blob) => {
        ...
        link.href = url;
        link.download = `excel-bancario-conciliacion-${period.reconciliation_number}-${period.cutoff_at.slice(0, 10)}.xlsx`;
        link.click();
      ...
      error: (error) => {
        ...
        this.set(
          this.errors,
          id,
          this.message(error, 'No fue posible descargar el Excel bancario.'),
        );
      },
    });
  }
  ...
  upload(id: string): void {
    ...
    if (!file) return;
    this.set(this.uploading, id, true);
    this.set(this.errors, id, '');
    this.set(this.successes, id, '');
    this.api.upload(file, id).subscribe({
      next: (result) => {
        this.set(this.uploading, id, false);
        this.clearSelectedFile(id);
        this.set(
          this.successes,
          id,
          result.replayed
            ...
            : `Conciliación procesada: ${result.row_count} movimientos.`,
        );
        this.loadPeriods();
      },
      error: (error) => {
        this.set(this.uploading, id, false);
        this.set(this.errors, id, this.message(error, 'El archivo fue rechazado.'));
      },
    });
  }
    ...
    this.api.pendingPeriods({ skipGlobalAlert: true }).subscribe({
      next: (periods) => {
        const sorted = [...periods].sort(
          (a, b) => new Date(b.cutoff_at).getTime() - new Date(a.cutoff_at).getTime(),
        );
      ...
      error: (error) => {
        ...
        this.globalError.set(
          this.message(error, 'No fue posible consultar las conciliaciones pendientes.'),
        );
      },
    });
  }
  ...
  private loadCount(id: string): void {
    ...
    this.api.simulatedTransfers(id, { skipGlobalAlert: true }).subscribe({
      next: (items) => this.set(this.counts, id, items.length),
      error: (error) =>
        this.set(
          this.errors,
          id,
          this.message(error, 'No fue posible consultar los movimientos de este corte.'),
        ),
    });
  }
  ...
  private set<T>(target: WritableSignal<Record<string, T>>, id: string, value: T): void {
