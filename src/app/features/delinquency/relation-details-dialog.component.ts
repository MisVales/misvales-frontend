      ...
      <div class="backdrop" role="dialog" aria-modal="true" aria-labelledby="dialog-title">
        <section class="dialog">
          <header class="dialog-header">
            <h2 id="dialog-title">Historial completo de relaciones</h2>
            <button type="button" class="close-btn" (click)="close.emit()" aria-label="Cerrar">
              &times;
            </button>
          </header>
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="border-b text-sm text-gray-500">
                  ...
                  <th class="py-2 font-semibold">Cobrado a clientes</th>
                  <th class="py-2 font-semibold">Ganancia distribuidora</th>
                  <th class="py-2 font-semibold">Neto a MisVales</th>
                  <th class="py-2 font-semibold">Pagado</th>
                ...
                @for (detail of relations; track detail.id) {
                  <tr class="border-b text-sm">
                    ...
                    <td class="py-3 text-red-600 font-medium">
                      {{ detail.payment_deadline_at | date: 'dd/MM/yyyy' }}
                    </td>
                    <td class="py-3">{{ detail.portfolio_total | currency: 'MXN' }}</td>
                    <td class="py-3 font-semibold text-emerald-700">
                      {{ detail.distributor_profit_total | currency: 'MXN' }}
                    </td>
                    <td class="py-3">{{ detail.misvales_total | currency: 'MXN' }}</td>
                    <td class="py-3">{{ detail.reconciled_total | currency: 'MXN' }}</td>
                      ...
                      <span
                        ...
                        [ngClass]="{
                          ...
                          'bg-yellow-100 text-yellow-800':
                            detail.financial_status === 'PARTIALLY_PAID',
                          'bg-slate-100 text-slate-800': detail.financial_status === 'PENDING',
                          'bg-red-100 text-red-800':
                            detail.financial_status === 'OVERDUE' ||
                            detail.financial_status === 'PAST_DUE' ||
                            detail.financial_status === 'ROLLED_FORWARD',
                        }"
                      >
                        {{ statusLabel(detail.financial_status) }}
                      </span>
                    </td>
