      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div class="flex gap-3" *ngIf="branch()">
          ...
          <button (click)="toggleStatus()" 
                  [disabled]="branch()!.isHeadquarters || facade.isLoading()"
                  [title]="branch()!.isHeadquarters ? 'No se puede desactivar la Sede Principal' : ''"
                  ...
                  [ngClass]="branch()!.isActive ? 'bg-white border-red-200 text-red-600 hover:bg-red-50' : 'bg-[#386641] border-[#386641] text-white hover:bg-[#6A994E]'">
      ...
      <div *ngIf="branch()" class="bg-white shadow-xl rounded-2xl p-8 border border-gray-100 flex flex-col md:flex-row gap-8">
        ...
        <div class="flex flex-col items-center justify-start shrink-0">
          ...
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold" 
                [ngClass]="branch()!.isHeadquarters ? 'bg-[#386641] text-white' : 'bg-gray-200 text-gray-700'">
            {{ branch()!.isHeadquarters ? 'Sede Principal' : 'Sucursal Foránea' }}
          </span>
        </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <p class="text-lg font-medium text-gray-900 mt-1 flex items-center gap-2">
                ...
                {{ branch()!.activeStaffCount }} Empleados
              </p>
            </div>
...
export class BranchDetail implements OnInit {
  ...
  toggleStatus() {
    ...
    if (!currentBranch || currentBranch.isHeadquarters) return;

    this.facade.toggleBranchStatus(currentBranch.id, !currentBranch.isActive, currentBranch.lockVersion);
  }
}
