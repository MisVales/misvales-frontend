...
import { NavItemData } from '../sidebar/sidebar.config';
  ...
  const items: readonly NavItemData[] = [
    { id: 'home', title: 'Inicio', icon: 'home', route: '/inicio' },
    {
      ...
      children: [
        ...
        { id: 'branch-detail', title: 'Detalle de sucursal', icon: 'map-pin', route: '/organizacion/sucursales/detalle' },
      ],
    },
  ];

  it('uses the most specific configured navigation label for detail URLs', () => {
    expect(findNavigationTitle(items, '/organizacion/sucursales/detalle/123')).toBe('Detalle de sucursal');
  });
