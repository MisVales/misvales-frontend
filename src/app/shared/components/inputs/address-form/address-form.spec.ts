...
import { AddressApiService } from '@core/api/address/address-api';
...
describe('AddressFormComponent suggestion semantics', () => {
  ...
  it('marks every missing field and focuses the first invalid control on submit', async () => {
    TestBed.configureTestingModule({
      imports: [AddressFormComponent],
      providers: [{
        provide: AddressApiService,
    ...
    expect(fixture.componentInstance.validarYEnfocarPrimerError()).toBe(false);
    await Promise.resolve();
    fixture.detectChanges();

    expect(fixture.nativeElement.textContent).toContain('El estado es obligatorio.');
    const stateControl = fixture.nativeElement.querySelector('[formControlName="state"]') as HTMLElement;
    expect(stateControl.contains(document.activeElement)).toBe(true);
  });
});
