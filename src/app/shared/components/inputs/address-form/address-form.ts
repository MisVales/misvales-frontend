import { Component, OnInit, inject, Input, Output, EventEmitter, OnDestroy, ElementRef } from '@angular/core';
import { FormBuilder, FormGroup, Validators, ReactiveFormsModule } from '@angular/forms';
import { CommonModule } from '@angular/common';
import { NgSelectModule } from '@ng-select/ng-select';
import { AddressApiService, State, Municipality, Colony } from '@core/api/address/address-api';
...
export class AddressFormComponent implements OnInit, OnDestroy {
  ...
  form: FormGroup = this.fb.group({
    ...
    exteriorNumber: [{ value: '', disabled: true }, [Validators.required, Validators.maxLength(20)]],
    interiorNumber: [{ value: '', disabled: true }, [Validators.maxLength(20)]]
  });
  ...
  validarYEnfocarPrimerError(): boolean {
    this.form.markAllAsTouched();
    Object.keys(this.form.controls).forEach((campo) => {
      if (this.form.get(campo)?.invalid) this.visibleErrorFields.add(campo);
    });
    this.emitChange();

    const primerCampoInvalido = Object.keys(this.form.controls)
      .find((campo) => this.form.get(campo)?.invalid);
    if (primerCampoInvalido) {
      queueMicrotask(() => {
        const host = this.host.nativeElement as HTMLElement;
        const control = host.querySelector(`[formControlName="${primerCampoInvalido}"]`) as HTMLElement | null;
        const elementoEnfocable = control?.matches('input, select, textarea, button')
          ? control
          : control?.querySelector('input, select, textarea, button') as HTMLElement | null;
